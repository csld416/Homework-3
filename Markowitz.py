...
class RiskParityPortfolio:
    ...
    def calculate_weights(self):
        ...
        for i in range(self.lookback + 1, len(df)):
            window = df_returns.loc[df.index[i - self.lookback : i], assets]
            ...
            # Create a full-row with 0.0 and then assign weights to only asset columns
    ...
    def mv_opt(self, R_n, gamma):
        ...
        with gp.Env(empty=True) as env:
            ...
            with gp.Model(env=env, name="portfolio") as model:
                ...
                w = model.addMVar(n, name="w", lb=0.0)  # lb=0 enforces long-only constraint

                # Objective: w.T @ mu - (gamma / 2) * w.T @ Sigma @ w
                quad_term = 0.5 * gamma * w @ Sigma @ w
                linear_term = mu @ w
